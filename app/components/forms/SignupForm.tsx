...
const SignupForm: React.FC<SignupFormProps> = ({setPage}) => {
    ...
    return <form onSubmit={handleSubmit} className="p-[32px]">
        <h1 className="mb-10 font-bold text-3xl">Create New Account</h1>
        ...
        <input autoComplete="off" name="confirmPassword" value={confirmPassword} onChange={e => {
            setSubmitted(false);
            setConfirmPassword(e.target.value);
        }} className="w-full mb-4 rounded-lg h-12 indent-3 focus:outline-none focus:border focus:border-[#4184F3] shadow-sm bg-gray-100" type={isChecked?"text":"password"} placeholder="Confirm Password" />
        ...
        <button disabled={email === "" || password === "" || loading} className="bg-[#4184F3] text-white w-full h-12 rounded-lg flex items-center justify-center shadow-sm">
          ...
          {!loading && !submitted && "Sign up"}
        </button>
        <p className="mt-4 flex gap-1">
          Alreadt have an account?<span className="underline font-semibold text-[#4184f3] cursor-pointer" onClick={() => setPage("Login")}>
            Log in
          </span>
        </p>
      </form>;
